...
/obj/machinery/newscaster/ui_data(mob/user, list/params)
	...
	// Устанавливаем канал по умолчанию если ничего не выбрано
	if(!selected_channel && GLOB.news_network.network_channels.len > 0)
		var/datum/feed_channel/default_channel = GLOB.news_network.network_channels[1]
		selected_channel = default_channel.channel_ID
	
	// Получаем каналы из глобальной сети
...
/obj/machinery/newscaster/ui_act(action, params)
	...
	switch(action)
		...
		if("create_channel")
			...
			GLOB.news_network.create_feed_channel(channel_name, scanned_user, channel_desc, locked)
		...
		if("create_wanted")
			...
			GLOB.news_network.submit_wanted(criminal_name, crime_desc, scanned_user, null, FALSE, TRUE)
		...
		if("clear_wanted")
			...
			if(wanted_id && GLOB.news_network.delete_wanted_by_id(wanted_id))
				to_chat(usr, span_notice("Розыск отменен."))
			else
				// Очистить все розыски
				...
				for(var/datum/feed_message/message in channel.messages)
					if(message.message_ID == message_id)
						if(message.locked)
							...
							return
						var/datum/feed_comment/new_comment = new /datum/feed_comment
		...
		if("create_story")
			...
			// Находим канал
			var/datum/feed_channel/target_channel = null
			for(var/datum/feed_channel/channel in GLOB.news_network.network_channels)
			...
			// Публикуем новость
			GLOB.news_network.submit_article(story_text, scanned_user, target_channel.channel_name, null, FALSE, TRUE, TRUE)
				...
				for(var/datum/feed_message/message in channel.messages)
					if(message.message_ID == message_id)
						...
						if(scanned_user in message.likes)
							...
							// Убираем дизлайк если есть и ставим лайк
				...
				for(var/datum/feed_message/message in channel.messages)
					if(message.message_ID == message_id)
						...
						if(scanned_user in message.dislikes)
							...
							// Убираем лайк если есть и ставим дизлайк
							if(scanned_user in message.likes)
								message.likes -= scanned_user
							message.dislikes += scanned_user
						. = TRUE
